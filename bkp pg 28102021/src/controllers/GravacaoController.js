...
class GravacaoController{
    async listarGravacoes(req,res){     
        ...
        for(let i=0; i<gravacoes.length; ++i){
            ...
            const idEmpresa = await User.codEmpresa(empresa)
    ...
    async gravacaoCompartilhada(req,res){
        ...
        const empresa = await Gravacao.prefixEmpresa(idEmpresa)
        if(empresa==false){
            res.json({"error":true,"message":"Gravação não encontrada!"})
            return false;
        }

        const gravacoes = await Gravacao.linkGravacao(empresa,idRec);
        ...
        const dataG = {}
              ...
              const nameEmpresa = {}
                    ...
                    nameEmpresa["value"]=await User.nomeEmpresa(empresa)
