...
class Campanhas{   
    ...
    async addMailingCampanha(idCampanha,idMailing){
        ...
        sql = `ALTER TABLE ${_dbConnection2.default.db.mailings}.${tabelaNumeros} 
    ...
    async removeMailingCampanha(idCampanha){
        ...
        sql = `ALTER TABLE ${_dbConnection2.default.db.mailings}.${infoMailing[0].tabela_numeros} DROP COLUMN campanha_${idCampanha}`
        await this.querySync(sql)
    ...
    async delFilterDial(tabela,idCampanha,tipo,valor,regiao){
        ...
        let sql = `UPDATE ${_dbConnection2.default.db.mailings}.${tabela} SET campanha_${idCampanha}=1 WHERE ${filter}`       
    ...
    async addFilterDial(tabela,idCampanha,tipo,valor,regiao){
        ...
        filter+=`${tipo}='${valor}'`
        if(regiao!=0){ filter+=` AND uf='${regiao}'`}       
        let sql = `UPDATE ${_dbConnection2.default.db.mailings}.${tabela} SET campanha_${idCampanha}=0 WHERE ${filter}`          
        await this.querySync(sql)
        return true
    }
    ...
    async totalNumeros(tabela,uf,ddd){
        ...
        const sql = `SELECT COUNT(id) AS total FROM ${_dbConnection2.default.db.mailings}.${tabela} WHERE valido=1 ${filter}` 
    ...
    async numerosFiltrados(idMailing,tabelaNumeros,idCampanha,uf){
        let filter=""
        if(uf!=0){ filter += ` AND uf="${uf}"` }
        const sql = `SELECT COUNT(id) AS total FROM ${_dbConnection2.default.db.mailings}.${tabelaNumeros} WHERE valido=1 AND campanha_${idCampanha}=1 ${filter}`
        const r = await this.querySync(sql)
        return r[0].total
    ...
    async dddsMailings(tabela,uf){
        ...
        let sql = `SELECT DISTINCT ddd FROM ${_dbConnection2.default.db.mailings}.${tabela} ${filter}`
        return await this.querySync(sql)
    }
    ...
    async mailingsContatadosPorCampanha(idCampanha,status){
        const sql = `SELECT count(id) AS total FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing 
    ...
    mailingsContatados(callback){
        const sql = `SELECT count(t.id) AS contatados 
                       FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing AS t 
