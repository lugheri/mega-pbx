...
class Dashboard{
    ...
    async painel(){
        return new Promise(async (resolve, reject) =>{
            ...
            //Contatados
            sql = `SELECT count(t.id) AS contatados FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing AS t JOIN campanhas AS c ON c.id=t.idCampanha WHERE t.contatado='S' AND c.estado=1 AND c.status=1`
            const rows_jaContatados = await this.querySync(sql)
            const contatados = parseInt(rows_jaContatados[0].contatados)

            //Nao Contatados
            sql = `SELECT count(t.id) AS nao_contatados FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing AS t JOIN campanhas AS c ON c.id=t.idCampanha WHERE t.contatado='N' AND c.estado=1 AND c.status=1`
