...
class Dashboard{
    ...
    querySync(sql,database){
        return new Promise((resolve, reject) =>{
           _dbConnection2.default.base(database).query(sql,(e,row)=>{
               if(e)
                reject(e)
    ...
    async painel(){
        return new Promise(async (resolve, reject) =>{
            ...
            const rows_historicoLogins = await this.querySync(sql,base)
            ...
            const rows_totalEmLigacao = await this.querySync(sql,base)
            ...
            const rows_totalEmPausa = await this.querySync(sql,base)
            ...
            const rows_totalDisponivel = await this.querySync(sql,base)
            ...
            const rows_usuariosEmLigacao = await this.querySync(sql,base)
            for(let i = 0; i < rows_usuariosEmLigacao.length; i++){
                ...
                const tempo = await this.querySync(sql,base)
                if(tempo.length==0){
            ...
            const rows_usuariosEmPausa = await this.querySync(sql,base)
            ...
            const rows_usuariosDisponiveis = await this.querySync(sql,base)
            ...
            const rows_campanhasAtivas = await this.querySync(sql,base)
            ...
            const rows_historicoCampanhas = await this.querySync(sql,base)
            ...
            const rows_totalDiscando = await this.querySync(sql,base)
            ...
            const rows_totalEmEspera = await this.querySync(sql,base)
            ...
            const rows_totalParadas = await this.querySync(sql,base)
            ...
            const rows_campanhasDiscando = await this.querySync(sql,base)
            ...
            const rows_campanhasAguardando = await this.querySync(sql,base)
            ...
            const rows_campanhasParadas = await this.querySync(sql,base)
            ...
            const rows_total_reg = await this.querySync(sql,base)
            let total
            ...
            //Contatados
            sql = `SELECT count(t.id) AS contatados FROM mailings.campanhas_tabulacao_mailing AS t JOIN campanhas AS c ON c.id=t.idCampanha WHERE t.contatado='S' AND c.estado=1 AND c.status=1`
            const rows_jaContatados = await this.querySync(sql,base)
            const contatados = parseInt(rows_jaContatados[0].contatados)

            //Nao Contatados
            sql = `SELECT count(t.id) AS nao_contatados FROM mailings.campanhas_tabulacao_mailing AS t JOIN campanhas AS c ON c.id=t.idCampanha WHERE t.contatado='N' AND c.estado=1 AND c.status=1`
            const rows_naoContatados = await this.querySync(sql,base)
            ...
            const row_chamadasSimultaneas = await this.querySync(sql,base)
            //CONECTADAS
            sql = `SELECT conectadas FROM log_chamadas_simultaneas ORDER BY id DESC LIMIT ${limit}`
            const row_chamadasConectadas = await this.querySync(sql,base)
