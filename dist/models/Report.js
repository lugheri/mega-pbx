...
class Report{
    ...
    async monitoramentoCampanhaIndividual(idCampanha,callback){
        ...
        sql=`SELECT count(id) as trabalhados FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing WHERE contatado='S' AND idCampanha=${idCampanha}`        
        ...
        sql=`SELECT count(id) as produtivo FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing WHERE contatado='S' AND produtivo=1 AND idCampanha=${idCampanha}`        
        ...
        sql=`SELECT count(id) as improdutivas FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing WHERE contatado='S' AND produtivo=0 AND idCampanha=${idCampanha}`        
    ...
    async monitoramentoCampanhaGeral(callback){
        ...
        sql=`SELECT count(id) as trabalhados FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing WHERE contatado='S'`        
        const rowsTrabalhadas = await this.querySync(sql)  
        let trabalhados=0
        let produtivas=0
        let improdutivas=0
        ...
        sql=`SELECT count(id) as produtivo FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing WHERE contatado='S' AND produtivo=1`        
        const rowsProdutivas = await this.querySync(sql)     
        if(produtivas>0){
        ...
        sql=`SELECT count(id) as improdutivas FROM ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing WHERE contatado='S' AND produtivo=0`        
        const rowsImprodutivas = await this.querySync(sql)   
        if(improdutivas>0){
             improdutivas = (rowsImprodutivas[0].improdutivas/rowsTrabalhadas[0].trabalhados)*100
        }
        ...
        return new Promise ((resolve,reject) =>{
            ...
            _dbConnection2.default.banco.query(sql,(e,rowDadosCampanha)=>{
                ...
                const sql = `SELECT COUNT(id_key_base) AS trabalhados 
                               FROM ${tabela} AS t 
                               LEFT JOIN ${_dbConnection2.default.db.mailings}.campanhas_tabulacao_mailing AS c ON c.idRegistro=t.id_key_base 
