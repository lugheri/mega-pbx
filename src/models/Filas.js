...
class Filas{
    ...
    async dadosFila(nomeFila){
        const sql = `SELECT * FROM ${connect.db.asterisk}.queues WHERE name='${nomeFila}'`
    ...
    async editarFila(nomeFila,dados){
        const sql = `UPDATE ${connect.db.asterisk}.queues SET musiconhold='${dados.musiconhold}',strategy='${dados.strategy}',timeout='${dados.timeout}',retry='${dados.retry}',autopause='${dados.autopause}',maxlen='${dados.maxlen}' WHERE name='${nomeFila}'`
        return await this.querySync(sql)
    }
